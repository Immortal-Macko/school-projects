...
static int	ft_check_string(char *string, char **line)
{
	...
	if (flag == -1)
		*line = ft_strjoin_my(line, string);
	if (flag >= 0)
	{
...
static int	ft_read_line(int fd, char *string, char **line, int i)
{
	int		check;
	char	*buf;

	check = -1;
	buf = ft_calloc(100 + 1, 1);
	if (!buf)
		return (-1);
	while (i < 0)
	{
...
int	get_next_line(int fd, char **line)
{
	...
	*line = ft_calloc(1, 1);
	if (!string)
		string = ft_calloc(100 + 1, 1);
	flag = ft_check_string(string, line);
	if (flag < 0)
		check = ft_read_line(fd, string, line, -1);
	if (check == -1 && *line)
		free(*line);
	if (check == -1 || check == 0)
	{
